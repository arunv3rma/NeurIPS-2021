...
def regret_plotting(regret, cases, plotting_info):
    ...
    for c in range(cases):
        ...
        #     print(plotting_info[4][c], horizon[i], batched_regret[i], error[i])
...
# #############################  Algorithms #############################
# UCB1 algorithm
def ucb1(mu, omega, sigma, sigma_w, T, b):
    ...
    for t in range(K, T):
        ...
        arm_ucb = arm_rewards/num_pulls + (b*np.sqrt((2.0*np.log(t))/num_pulls))
...
# UCB-V algorithm
def ucb_v(mu, omega, sigma, sigma_w, T, b):
    K                   = len(mu)           # Number of arms
    arm_rewards         = np.zeros(K)       # Collected rewards for arms
    ...
    for t in range(K, T):
        ...
        # Calculating the UCBs for each arm
        arm_ucb = mu_est + np.sqrt((2.2*np.log(t)*sample_var)/num_pulls) + ((3.3*b*np.log(t))/num_pulls)
...
# EUCBV algorithm
def eucbv(mu, omega, sigma, sigma_w, T, b):
    K                   = len(mu)           # Number of arms
    arm_rewards         = np.zeros(K)       # Collected rewards for arms
    arm_rewards_seq     = np.zeros(K)       # Collected sequare of rewards for arms
    num_pulls           = np.zeros(K)       # Number of arm pulls
    ...
    for t in range(K, T):
        ...
        # Calculating the UCBs for each arm
        arm_ucb = mu_est + (b*np.sqrt( ((sample_var + 2.0) * np.log(T/K)) / (4.0*num_pulls) ))
...
def ucb_cv(mu, omega, sigma, sigma_w, T, est_omega):
    K                   = len(mu)           # Number of arms
    arm_rewards         = np.zeros(K)       # Collected rewards for arms
    arm_rewards_seq     = np.zeros(K)       # Collected sequare of rewards for arms
    ...
    for t in range(3*K, T):
        ...
        cv_centered_seq = arm_cv_seq + (num_pulls*(est_omega**2)) - (2.0*est_omega*arm_cv)

        # Computing beta value
        beta = (arm_cross_terms - (est_omega*arm_rewards) - (mu_est*arm_cv) + (num_pulls*mu_est*est_omega))/cv_centered_seq

        # Estimated mean of new estimator
        mu_cv_est = mu_est + (beta*est_omega) - (beta*cv_est)

        # Computing sum of sequare of new observation
        cv_rewards_seq = arm_rewards_seq + (beta*beta*cv_centered_seq) - (2*beta*arm_cross_terms) + (2*beta*est_omega*arm_rewards)

        # Computing sample variance of arms
        sample_var = (1.0/(num_pulls - 2)) * (cv_rewards_seq - (num_pulls*mu_cv_est*mu_cv_est) )

        # Multiplier of sample variance to get variance of new estimator
        var_mult = (1.0/num_pulls) / (1.0 - ( ((arm_cv - (num_pulls*est_omega))**2) / (num_pulls*cv_centered_seq)) )
...
arm_gap         = 0.5
cv_gap          = 0.5
rewards_sup     = 2*(3*max_arm_var)     # It will needed for UCB based algorithms
...
# Runnging algorithm
cv_error_list   = [3, 2, 1, 0]
algs            = ['UCB1', 'UCB-V', 'TS', 'EUCBV']
total_algs      = len(algs)
cases           = [r'UCB-CV $(\epsilon = $' + str(cv_error_list[c]) + ')' for c in range(len(cv_error_list))]
...
for _ in tqdm(range(runs)):
    ...
    for a in range(total_algs):
        if algs[a] == 'UCB1':
            iter_regret = ucb1(arms_mean, cv_mean, arms_var, cv_var, samples, rewards_sup)

        elif algs[a] == 'UCB-V':
            iter_regret = ucb_v(arms_mean, cv_mean, arms_var, cv_var, samples, rewards_sup)
        
        elif algs[a] == 'EUCBV':
            iter_regret = eucbv(arms_mean, cv_mean, arms_var, cv_var, samples, rewards_sup)
    ...
    for c in range(total_cases):
        iter_regret = ucb_cv(arms_mean, cv_mean, arms_var, cv_var, samples, cv_mean + cv_error_list[c])
        run_regret.append(iter_regret)
...
xlabel              = "Rounds"
ylabel              = "Regret"
file_to_save        = "figure4c.png"
title               = "Comparison of Algorithms"
save_to_path        = "plots/" 
location_to_save    = save_to_path + file_to_save
plotting_parameters = [xlabel, ylabel, title, location_to_save, algs + cases, samples]

# Regret Plotting
regret_plotting(algos_regret, total_cases + total_algs, plotting_parameters)