...
arms_sigma  = np.zeros(arms)
cv_mean     = np.zeros(arms)
cv_sigma      = np.zeros(arms)
for k in range(arms):
...
for _ in tqdm(range(runs)):
    ...
    for c in range(total_cases):
        if cases[c] == 'Batching':
            iter_regret = bacthing(arms_mean, cv_mean, arms_sigma, cv_sigma, samples)

        elif cases[c] == 'Jackknifing':
            iter_regret = jackknifing(arms_mean, cv_mean, arms_sigma, cv_sigma, samples)
        
        elif cases[c] == 'Splitting':
            iter_regret = splitting(arms_mean, cv_mean, arms_sigma, cv_sigma, samples)
        
        elif cases[c] == 'UCB-CV':
            iter_regret = ucb_cv(arms_mean, cv_mean, arms_sigma, cv_sigma, samples)
