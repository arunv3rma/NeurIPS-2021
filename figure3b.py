...
def jackknifing(mu, omega, scale_m, scale_w, T):
    K                   = len(mu)           # Number of arms
    ...
    max_mean_reward     = max(mu)           # Maximum mean reward
    ...
    for s in range(2*K):
        # Samples
        k = s % K
        random_sample = np.random.gamma(mu[k], scale=scale_m[k], size=None)
        arm_cv_value = np.random.gamma(omega[k], scale=scale_w[k], size=None)
        arm_reward = random_sample + arm_cv_value
        ...
        # Regret
        instantaneous_regret.append(max_mean_reward - mu[k])
    ...
    for t in range(2*K, T):
        ...
        # Samples
        random_sample = np.random.gamma(mu[I_t], scale=scale_m[I_t], size=None)
        arm_cv_value = np.random.gamma(omega[I_t], scale=scale_w[I_t], size=None)
        arm_reward = random_sample + arm_cv_value
        ...
        # Regret
        instantaneous_regret.append(max_mean_reward - mu[I_t])
...
def splitting(mu, omega, scale_m, scale_w, T):
    K                   = len(mu)           # Number of arms
...
arms_scale  = np.zeros(arms)
cv_mean     = np.zeros(arms)
cv_scale      = np.zeros(arms)
for k in range(arms):
...
for _ in tqdm(range(runs)):
    ...
    for c in range(total_cases):
        if cases[c] == 'Batching':
            iter_regret = bacthing(arms_mean, cv_mean, arms_scale, cv_scale, samples)

        elif cases[c] == 'Jackknifing':
            iter_regret = jackknifing(arms_mean, cv_mean, arms_scale, cv_scale, samples)
        
        elif cases[c] == 'Splitting':
            iter_regret = splitting(arms_mean, cv_mean, arms_scale, cv_scale, samples)
        
        elif cases[c] == 'UCB-CV':
            iter_regret = ucb_cv(arms_mean, cv_mean, arms_scale, cv_scale, samples)
